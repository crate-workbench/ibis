...
pytest.importorskip("crate")
sa = pytest.importorskip("sqlalchemy")

import crate.client.sqlalchemy.dialect as cratedb_dialect
# noqa: E402

CRATEDB_TEST_DB = os.environ.get("IBIS_TEST_CRATEDB_DATABASE", "ibis_testing")
IBIS_CRATEDB_HOST = os.environ.get("IBIS_TEST_CRATEDB_HOST", "localhost")
IBIS_CRATEDB_PORT = os.environ.get("IBIS_TEST_CRATEDB_PORT", "4200")
IBIS_CRATEDB_USER = os.environ.get("IBIS_TEST_CRATEDB_USER", "crate")
IBIS_CRATEDB_PASS = os.environ.get("IBIS_TEST_CRATEDB_PASSWORD", "")


def test_foo(alltypes):
    schema = [
        ("id", "int32"),
        ("bool_col", "boolean"),
        ("tinyint_col", "int8"),
    ...
    ibis.table(name="foo", schema=schema)
    #tbl.
    #popo
...
def test_list_databases(con):
    assert CRATEDB_TEST_DB is not None
    assert CRATEDB_TEST_DB in con.list_databases()
...
@pytest.mark.parametrize("params", [{}, {"database": CRATEDB_TEST_DB}])
