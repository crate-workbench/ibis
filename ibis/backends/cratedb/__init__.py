...
from ibis.backends.cratedb.compiler import CrateDBCompiler
...
class Backend(BaseAlchemyBackend, AlchemyCanCreateSchema):
    ...
    def do_connect(
        ...
        """
        if driver != "crate":
            raise NotImplementedError("crate is currently the only supported driver")
        ...
        # FIXME
        # TypeError: 'Cursor' object does not support the context manager protocol
        """
...
"""
        ...
        def split_name_type(arg: str) -> tuple[str, dt.DataType]:
            ...
            return name, CrateDBType.from_string(typ)
        ...
        [row] = rows
        return_type = CrateDBType.from_string(row["return_type"])
...
ORDER BY attnum"""
        ...
        with self.begin() as con:
            ...
            try:
                yield from (
                    (col, CrateDBType.from_string(typestr))
